...
def load_data():
    ...
    if not df.empty:
        ...
        # 💡 전략 분류: 거래대금 폭발(등락률 3%↑) 또는 BB상단 돌파 시 '창(공격)', 나머지는 '방패(수비)'
        df['전략분류'] = df.apply(lambda x: '창' if (x['change_rate'] > 3 or x['last_price'] > x['bb_upper']) else '방패', axis=1)
        
        # 💡 [신규 추가] 판별 포인트: 창과 방패로 나뉜 구체적인 '원인'을 생성
        # 💡 [판별 포인트] 로직 고도화: 전환 시그널 & 매집 파악
        def get_strat_reason(row):
            ...
            if row['rsi'] < 30: reasons.append("과매도구간")
            if row['r_square'] > 0.8: reasons.append("추세강화")

            # 2. 방패 -> 창 (전환 임박 시그널)
            # 하락추세강화(R2고점)였다가 추세가 깨지며(R2저점) 가격이 반등할 때
            if row['전략분류'] == '방패' and row['r_square'] < 0.4 and row['change_rate'] > 0:
                reasons.append("✨추세반전(매수타점)")
                
            # 3. 세력 매집 흔적 (잠재력)
            # 주가는 크게 안 오르는데 RSI가 바닥에서 고개를 들거나, 특정 구간 횡보 시
            if -1 < row['change_rate'] < 1 and row['rsi'] > 50 and row['r_square'] < 0.2:
                reasons.append("📦조용한매집중")
                
            if row['rsi'] < 30: reasons.append("📉과매도구간")
 
            if row['전략분류'] == '창':
                return " & ".join(reasons) if reasons else "주도주흐름"
            else:
                return "박스권유지" if not reasons else " & ".join(reasons)
...
with col_btn:
    # 버튼을 타이틀 높이와 맞추기 위해 상단에 여백(Padding)을 살짝 줍니다.
    st.write("##") # 타이틀 높이와 맞추기 위한 빈 공간 생성
    if st.button("🔄 테마 동기화", use_container_width=True):
        from naver_crawler import sync_naver_themes # 호출 시점에 임포트
...
if not df.empty:
    ...
    with tab1:
        ...
        for _, row in st_df.iterrows():
            ...
            with r_cols[11]:
                abs_val = min(abs(rate), 15)
                width_pct = (abs_val / 15) * 50
                ...
                with t_cols[idx]:
                    ...
                    with st.container(border=True):
                        ...
                        for _, row in t_stocks.iterrows():
                            ...
                            abs_v = min(abs(r), 15); w = (abs_v / 15) * 50; l = 50 if r > 0 else 50 - w
                            st.markdown(f"<div style='display:flex; justify-content:space-between; font-size:0.85em; margin-top:5px;'><b>{row['종목명(코드)']}</b><span style='color:{c}'>{r:+.2f}%</span></div>", unsafe_allow_html=True)
                            st.markdown(f'<div class="gauge-container"><div class="gauge-center-line"></div><div style="position:absolute; background-color:{c}; height:100%; width:{w}%; left:{l}%; border-radius:2px;"></div></div>', unsafe_allow_html=True)
else:
    st.info("데이터 로딩 중...")