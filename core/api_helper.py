...
class KISApiHelper:
    ...
    def get_my_interests(self):
        """
        [최종판] 한투 API 문서 기준 관심종목 조회 2단계 로직
        1. 그룹 목록 조회 -> 2. 첫 번째 그룹의 종목 리스트 추출
        """
        # --- [준비 단계] ---
        self.auth()
        ...
        try:
            ...
            if stock_data.get('rt_cd') == '0':
                ...
                print(f"✅ 최종 {len(codes)}개 종목 로드 완료: {codes}")
                return codes
            else:
