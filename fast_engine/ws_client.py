...
class KISWebsocketClient:
    ...
    async def connect(self, callback):
        while True:
            ...
            if not self.is_market_operating():
                current_str = datetime.now(KST).strftime('%H:%M:%S')
                print(f"😴 [장외 대기] {current_str} - 서버가 닫힌 시간입니다. 1분 후 재확인합니다.")
            ...
            try:
                ...
                # 6. 에러 발생 시 타임스탬프와 함께 출력 (PM2 로그 확인용)
                print(f"🚨 [WS 에러] {datetime.now(KST).strftime('%H:%M:%S')} : {e}")
                print("🔄 10초 후 재연결 모드로 돌아갑니다...")
    ...
    def send_telegram(message: str) -> bool:
        ...
        TELEGRAM_CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "838961911")
        try:
